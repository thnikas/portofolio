    ...
    <nav className={`${styles.paddingX} w-full flex items-center py-5 fixed top-0 z-20 bg-primary`}>
      <div className='w-full flex justify-between items-center max-w-7x mx-auto'>
        ...
        >
          ...
          <p className='text-white text=[18px] flex font-bold cursor-pointer'>Thomas&nbsp; 
            <span className='sm:block hidden'>|&nbsp;Nika</span>
          </p>
        </Link>
        <ul className='list-none hidden sm:flex flex-row gap-10'>
          {navLinks.map((link)=>(
            ...
            >
             <a href={`#${link.id}`}> <span className='font-bold roboto pr-[3%]'>{'<'}</span>{link.title}<span className='roboto pl-[3%] font-bold'>{'/>'}</span></a>
            </li>
          ))}
        </ul>
